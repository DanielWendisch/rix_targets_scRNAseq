...
rix(
  ...
  r_pkgs = c("reticulate",
    ...
    "gt",
    "fs",
    "languageserver", # required for R in Visual Studio Code over WSL
    "targets",
    "tarchetypes", # needed for e.g. tar_plan
    "rix", # to run this file from the created environment
    "qs2", # not exactly sure why needed and not provided by targets
    "qs", # not exactly sure why needed and not provided by targets
    # dependencies but probably because of tar_option_set(packages = c(.. ),  format = "qs")
    "visNetwork", # needed for tar_visNetwork() of targets package, 
    #not sure why dependencies arent installed automatically
    "usethis"
